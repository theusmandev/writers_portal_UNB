--  Writers Portal — Migration 015: Publish Episodes
--  File: supabase/migrations/015_publish_episodes.sql
--
--  HOW TO RUN:
--    Supabase Dashboard → SQL Editor → New Query → paste → Run
-- ============================================================

-- 1. Add published column to episodes
ALTER TABLE episodes ADD COLUMN IF NOT EXISTS published boolean DEFAULT false;

-- 2. Drop existing function so we can change the return type
DROP FUNCTION IF EXISTS get_submission_by_code(text, text);

-- 3. Recreate get_submission_by_code with published field in episodes array
